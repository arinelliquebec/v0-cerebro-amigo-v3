...
///
/// Configuração via env vars planas (estilo Program.cs existente):
///   RESEND_API_KEY  (obrigatório)
///   RESEND_FROM     (ex.: "Cérebro Amigo &lt;contato@cerebroamigo.com.br&gt;")
/// </summary>
public sealed class ResendClient
{
    ...
    public ResendClient(HttpClient http, IConfiguration cfg, ILogger<ResendClient> logger)
    {
        ...
        _from = cfg["RESEND_FROM"]
            ?? throw new InvalidOperationException(
                "RESEND_FROM não configurada (ex.: 'Cérebro Amigo <contato@cerebroamigo.com.br>').");

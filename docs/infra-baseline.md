...
3. **Resize `t3.large → t3.medium`:** ✅ **JUSTIFICADO nos dois gates (2026-06-21), NÃO aplicado** (aguarda "go" do Patrick). **Capacidade:** pico 1,27 GB + steady 1,0 GB cabem com folga em 4 GB (~2,7 GB livres). **Custo (SP, §5):** in-family → SP segue cobrindo; descer puxa web+checkup (hoje on-demand) p/ debaixo do SP → **~$38/mês** de economia efetiva, commitment ~100% usado (nada encalha). Aplicar = `aws ec2 stop-instances` → `modify-instance-attribute --instance-type t3.medium` → `start-instances` (downtime de stop/start; ADR-072 já assume deploy com downtime). **Depende de web+checkup seguirem t3.small** (são eles que absorvem o commit liberado).

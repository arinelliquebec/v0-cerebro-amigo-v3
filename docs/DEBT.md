...
| T0-8 | **Retenção de backup curta e postura de DR do RDS** (dado de saúde, categoria especial LGPD) | ~~🟡 Média~~ ✅ **Resolvido (config) / 🟡 residual** | O RDS `cerebro-postgres-enc` (sa-east-1, Single-AZ, cifrado ADR-018) estava com `BackupRetentionPeriod=7` — PITR curto p/ dado de saúde com obrigação de guarda longa. **Ajustado em prod 2026-06-25:** retention **7→35 dias** (máximo do PITR automático; sem downtime), janela de backup `01:23` → **`07:00-07:30 UTC`** (04:00-04:30 BRT, madrugada/menor uso, sem sobrepor manutenção `thu 06:01-06:31`). **Region auditada:** snapshots automáticos em `sa-east-1b`, **sem** replicação cross-region, `us-east-1` vazio = residência BR correta. Runbook `backup-e-restore.md` reescrito (instância certa `cerebro-postgres-enc`; cadência snapshot 1×/dia + WAL ~5min → **RPO ≈ 5min**; **cross-region snapshot VEDADO** por residência LGPD — AWS só tem 1 region no BR). PRs #151/#152. | **Residual:** (1) **guarda > 35d** (arquivo de prontuário / LGPD longo prazo) **não provisionada** → **AWS Backup vault** in-region (`sa-east-1`, lifecycle em anos, compliance-lock) ou snapshots manuais retidos. (2) **Single-AZ = SPOF** (sem failover intra-region); religar **Multi-AZ** no gatilho 1º pagante/Founders Hub. (3) Sem **alarme CloudWatch** "nenhum snapshot `available` em 7d" (sugerido no runbook, não criado). DR cross-region é **impossível** (1 só region BR) → restrição aceita, não é dívida. |
...
| T1-9 | **`CHECKUP_DATABASE_URL` morto nos containers clínicos, com creds master** | 🟡 Média | Auditoria de roles 2026-06-24 (pós-incidente 28P01 [[incident-gateway-db-auth-28P01]]): os 5 containers do box clínico (gateway, web, notifier, orchestrator, agents) recebem `CHECKUP_DATABASE_URL` via `.env`/compose, mas **nenhum código clínico o lê** (grep: só `apps/checkup/src/lib/db` + ASG/compose do checkup o consomem). É config morta no box clínico (checkup roda em ASG próprio, ADR-045, com seu DSN via SSM `checkup_app`). Pior: os containers em runtime carregam o valor **velho com role `cerebroadmin` (master)** — master creds em repouso no env de 5 serviços que não tocam o schema `checkup` = blast-radius full-RDS se um container vazar. O `.env` (mtime 12:05Z) **já** foi corrigido p/ `checkup_app`; o master só sai dos containers no próximo recreate. | (1) Remover `CHECKUP_DATABASE_URL` do `.env`/compose **dos serviços clínicos** (mantê-lo só no path do checkup). (2) Garantir que o DSN do checkup nunca use `cerebroadmin` (least-privilege: `checkup_app` basta; master só p/ DDL/migration de schema, e aí num path dedicado, não no env de runtime). (3) `--force-recreate` p/ purgar o master dos containers vivos (ou deixar o próximo deploy fazer). |

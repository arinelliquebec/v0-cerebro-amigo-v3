...
| CK-4 | **`test_results` (consentido) + `report_emails` (e-mail PDF)** | ~~🟡 Média~~ ✅ **Parcial** | Descoberto que NÃO existiam (só no schema, sem rota). **Parte A FEITA + verificada E2E em prod (commit 00b2e37):** checkbox de consentimento (default off) → `POST /api/result` grava `test_results` só com `consented:true` (anônimo, sem PII); `consented:false`→400, 0 rows. Bug junto corrigido: schema tinha `total_score` como bigserial (insert falharia) → `integer()`. **Parte B PENDENTE:** e-mail → enviar PDF + `report_emails` (hash), via **AWS SES sa-east-1** (in-region, não Resend que é US). Bloqueio: SES está em **sandbox** (`ProductionAccessEnabled:false`) → precisa de production-access (pedido na AWS, lead ~24h) + verificação de domínio (DKIM no DNS da Vercel). | Parte B: criar identidade SES + DKIM, IAM `ses:SendEmail`, rota `/api/email-report` (bcrypt hash + send), pedir sandbox-exit. |

...
| CK-6 | **Key Anthropic do checkup é COMPARTILHADA com a clínica** | 🔴 Alta | Spend limit configurado ($25 teto / $18 alerta), MAS `CHECKUP_ANTHROPIC_API_KEY` == `ANTHROPIC_API_KEY` (mesma key, hash idêntico confirmado 2026-06-11). Budget compartilhado: abuso/pico no checkup (superfície pública) pode estourar o teto e **derrubar o LLM clínico** (crise, auditoria, conversa) — viola a regra #4 do `checkup/CLAUDE.md` e contradiz o design (compose já documenta key "PRÓPRIA e separada"). O atalho de MVP furou o isolamento. | Workspace "checkup" separado no Console Anthropic → key própria com spend limit próprio (os $25/$18 ficam lá) → trocar `CHECKUP_ANTHROPIC_API_KEY` no `.env` do EC2. Clínica fica com teto próprio (maior). Assim o checkup no pior caso só cai pro fallback estático, sem afetar o clínico. |

...
| # | Item | Severidade | Rationale | Caminho para resolver |
|---|---|---|---|---|
| CK-1 | **Sem observabilidade/alerta de erro no checkup** | 🔴 Alta | O `/api/pdf` retornava 500 em runtime e ficou **invisível até o smoke manual** — superfície pública sem monitoramento. Sentry foi tentado e revertido; Datadog avaliado e **parado por ora** (LGPD: Datadog ingere fora do BR). **Escopo:** isto é só do CHECKUP — os 4 serviços Python já têm observabilidade (LangSmith na camada LLM + Sentry nos erros de app); o checkup NÃO usa LangChain/LangSmith (Next.js + Anthropic SDK direto), então LangSmith não cobre o 500 do PDF. | Opção leve interina sem mandar dado pra fora: alerta de taxa de 5xx em `/api/*` via o watchdog existente do EC2 (que já bate em `/health`) + garantir que erros vão pro stderr (vão). Datadog/observabilidade externa = decisão futura (LGPD + região). |

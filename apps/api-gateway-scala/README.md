...
- `POSTGRES_JDBC_URL` + `POSTGRES_USER` + `POSTGRES_PASSWORD` — **ou** `POSTGRES_DSN`
  (formato Npgsql; convertido p/ JDBC automaticamente). Role = `cerebro_gateway`.
...
- [ ] **Ainda não compilado neste ambiente** (sem sbt/JDK no box) — **o 1º push é a 1ª compilação real**; versões em `build.sbt` podem precisar de bump (o job vai acusar).

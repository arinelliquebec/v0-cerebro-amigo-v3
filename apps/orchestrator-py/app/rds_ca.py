"""SSL verify-full para o RDS (T1-4, anti-MITM).

CA bundle da AWS RDS sa-east-1 embarcado (mesma fonte do checkup —
``apps/checkup/src/lib/db/rds-ca.ts``):
https://truststore.pki.rds.amazonaws.com/sa-east-1/sa-east-1-bundle.pem

Hosts ``*.rds.amazonaws.com`` conectam validando a cadeia (CA regional) E o
hostname do servidor. Qualquer outro host (localhost, ``postgres`` do compose,
Testcontainers) segue o ``sslmode`` da própria DSN — dev/CI intactos.
Atualizar o bundle quando a AWS rotacionar as CAs regionais.
"""

from __future__ import annotations

import ssl
import tempfile
from pathlib import Path
from urllib.parse import parse_qsl, urlencode, urlsplit, urlunsplit

RDS_CA_SA_EAST_1_PEM = """-----BEGIN CERTIFICATE-----

BLl1W/UsvZo6ldLIpoMbbrb5
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

Ou/Ay+ns+J+GyvyDUjBpVVW1
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

nWS0
-----END CERTIFICATE-----"""


def is_rds_host(dsn: str) -> bool:
    host = urlsplit(dsn).hostname or ""
    return host.endswith(".rds.amazonaws.com")


def ssl_context_for_dsn(dsn: str) -> ssl.SSLContext | None:
    """SSLContext verify-full para hosts RDS; None para o resto (dev/CI)."""
    if not is_rds_host(dsn):
        return None
    ctx = ssl.create_default_context(cadata=RDS_CA_SA_EAST_1_PEM)
    ctx.check_hostname = True
    ctx.verify_mode = ssl.CERT_REQUIRED
    return ctx


def _rootcert_path() -> Path:
    """Materializa o bundle em arquivo — libpq/psycopg só aceita caminho."""
    path = Path(tempfile.gettempdir()) / "rds-sa-east-1-bundle.pem"
    if not path.exists() or path.read_text() != RDS_CA_SA_EAST_1_PEM:
        path.write_text(RDS_CA_SA_EAST_1_PEM)
    return path


def verify_full_dsn(dsn: str) -> str:
    """Reescreve DSN URL de host RDS p/ sslmode=verify-full + sslrootcert.

    Usada na DSN do checkpointer do LangGraph (driver psycopg3/libpq, que não
    recebe SSLContext). DSNs de hosts não-RDS voltam intactas.
    """
    if not is_rds_host(dsn):
        return dsn
    parts = urlsplit(dsn)
    query = dict(parse_qsl(parts.query))
    query["sslmode"] = "verify-full"
    query["sslrootcert"] = str(_rootcert_path())
    return urlunsplit(parts._replace(query=urlencode(query)))

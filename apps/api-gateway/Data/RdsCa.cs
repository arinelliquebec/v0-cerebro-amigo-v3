using Npgsql;

namespace ApiGateway.Data;

/// <summary>
/// SSL verify-full para o RDS (DEBT T1-4, anti-MITM).
///
/// CA bundle da AWS RDS sa-east-1 embarcado (mesma fonte do checkup —
/// apps/checkup/src/lib/db/rds-ca.ts):
/// https://truststore.pki.rds.amazonaws.com/sa-east-1/sa-east-1-bundle.pem
///
/// Hosts *.rds.amazonaws.com conectam validando a cadeia (CA regional) E o
/// hostname do servidor. Qualquer outro host (localhost, Testcontainers)
/// mantém a connection string original — dev/CI intactos.
/// Atualizar o bundle quando a AWS rotacionar as CAs regionais.
/// </summary>
public static class RdsCa
{
    /// <summary>
    /// Sobe a connection string para SSL Mode=VerifyFull + Root Certificate
    /// quando o host é RDS. Npgsql exige o CA em arquivo, então o bundle é
    /// materializado no temp dir no primeiro uso.
    /// </summary>
    public static string UpgradeToVerifyFull(string connectionString)
    {
        var csb = new NpgsqlConnectionStringBuilder(connectionString);
        if (csb.Host is null ||
            !csb.Host.EndsWith(".rds.amazonaws.com", StringComparison.OrdinalIgnoreCase))
            return connectionString;

        var path = Path.Combine(Path.GetTempPath(), "rds-sa-east-1-bundle.pem");
        if (!File.Exists(path) || File.ReadAllText(path) != SaEast1BundlePem)
            File.WriteAllText(path, SaEast1BundlePem);

        csb.SslMode = SslMode.VerifyFull;
        csb.RootCertificate = path;
        csb.TrustServerCertificate = false;
        return csb.ConnectionString;
    }

    public const string SaEast1BundlePem = """
-----BEGIN CERTIFICATE-----

BLl1W/UsvZo6ldLIpoMbbrb5
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

Ou/Ay+ns+J+GyvyDUjBpVVW1
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

nWS0
-----END CERTIFICATE-----
""";
}
